  ...
  return <div className="design-gallery">
    ...
    <main>
      ...
      <section className="gallery-grid" aria-label="Five design previews">{directions.filter((d) => Number(d.id) < 6).map((d) => <article className={`gallery-card gallery-card-${d.id}`} key={d.id}>
        ...
        <div className="gallery-thumbnail"><div className="gallery-miniature" inert aria-hidden="true"><div className={`editorial ed-${d.theme}`}><DesignHeader compact /><DesignHero theme={d.theme} preview /></div></div><Link className="gallery-preview-link" href={`/designs/${d.id}`} aria-label={`Explore option ${d.id}: ${d.name}`}><span>Explore design 0{d.id} <SymbolIcon symbol="↗" /></span></Link></div>
        <div className="gallery-card-info"><div><h2><Link href={`/designs/${d.id}`}>{d.name}</Link></h2><p className="gallery-short">{d.short}</p></div><div className="gallery-swatches" aria-label="Colour palette">{d.palette.map((color) => <span key={color} style={{ backgroundColor: color }} />)}</div></div><p className="gallery-description">{d.description}</p><Link href={`/designs/${d.id}`} className="gallery-open">View full design <span aria-hidden="true"><SymbolIcon symbol="↗" /></span></Link>
      </article>)}</section>

import Navbar from "../../components/Navbar";
import Footer from "../../components/Footer";
import Link from "next/link";
import Reveal from "../../components/Reveal";

export const metadata = {
  title: {
    absolute: "Healthcare Web Design UK | Crea Web",
  },
  description:
    "Crea Web provides professional healthcare web design in the UK, creating secure, SEO-friendly websites for clinics, hospitals, dentists, and healthcare providers.",
  alternates: { canonical: "https://creaweb.co.uk/industries/healthcare" },
  openGraph: {
    title: "Healthcare Web Design UK | Crea Web",
    description: "Crea Web provides professional healthcare web design in the UK, creating secure, SEO-friendly websites for clinics, hospitals, dentists, and healthcare providers.",
    url: "https://creaweb.co.uk/industries/healthcare",
    type: "website",
  },
};

export default function HealthcareWebDesignPage() {
  const faqs = [
    {
      q: "How much does a healthcare website cost?",
      a: "Every healthcare website is tailored to the needs of your practice. Pricing depends on functionality, page count, and integrations. Contact us for a personalised quotation.",
    },
    {
      q: "Can patients book appointments online?",
      a: "Yes. We can integrate online appointment booking systems or connect your existing booking platform.",
    },
    {
      q: "Are your healthcare websites mobile-friendly?",
      a: "Absolutely. Every website is fully responsive and optimised for smartphones, tablets, and desktop devices.",
    },
    {
      q: "Do you provide SEO for healthcare websites?",
      a: "Yes. Every website includes technical SEO foundations, with optional ongoing SEO services available to help improve search visibility.",
    },
    {
      q: "Can you redesign our existing healthcare website?",
      a: "Yes. We regularly redesign outdated medical websites while improving performance, user experience, and search engine optimisation.",
    },
    {
      q: "Is my website secure?",
      a: "Security is a priority. We implement SSL encryption, secure hosting recommendations, and industry-standard development practices.",
    },
    {
      q: "Do you provide ongoing website maintenance?",
      a: "Yes. We offer flexible website maintenance and support plans to keep your website secure, updated, and performing at its best.",
    },
    {
      q: "Can you build websites for multiple clinic locations?",
      a: "Yes. We can create dedicated location pages, practitioner profiles, and service pages for multi-location healthcare organisations.",
    },
  ];

  const reasons = [
    {
      title: "Patient-Focused User Experience",
      description: "We create websites that are simple to navigate, helping patients quickly find treatments, locations, opening hours, and contact information.",
      icon: (
        <svg className="w-8 h-8 text-[#B08D57]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
      ),
    },
    {
      title: "Responsive Across Every Device",
      description: "Patients increasingly search for healthcare services on their phones. Every website we build performs beautifully across desktop, tablet, and mobile.",
      icon: (
        <svg className="w-8 h-8 text-[#B08D57]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>
      ),
    },
    {
      title: "SEO Built In",
      description: "Our healthcare websites are optimised to help clinics rank for local searches such as Private Dentist London, Physiotherapist Manchester, Medical Clinic Birmingham.",
      icon: (
        <svg className="w-8 h-8 text-[#B08D57]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
      ),
    },
    {
      title: "Fast & Secure",
      description: "Performance and reliability matter in healthcare. We build websites that load quickly, follow modern security practices, and deliver dependable user experience.",
      icon: (
        <svg className="w-8 h-8 text-[#B08D57]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
      ),
    },
    {
      title: "Designed Around Your Practice",
      description: "No templates. Every healthcare website is designed specifically around your services, branding, patient journey, and long-term goals.",
      icon: (
        <svg className="w-8 h-8 text-[#B08D57]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
      ),
    },
    {
      title: "Ongoing Support & Partnership",
      description: "We don't disappear after launch. Ongoing maintenance, updates, and strategic guidance help your practice stay ahead as your business grows.",
      icon: (
        <svg className="w-8 h-8 text-[#B08D57]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2m-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>
      ),
    },
  ];

  const features = [
    "Online appointment booking integration",
    "Practitioner profile pages",
    "Treatment and service pages",
    "Patient information resources",
    "FAQ sections",
    "Secure contact forms",
    "Interactive Google Maps",
    "Reviews & testimonials",
    "Private patient enquiry forms",
    "Emergency contact information",
    "Blog & healthcare articles",
    "Multi-location practice pages",
    "Accessibility-friendly layouts",
    "GDPR-ready forms",
    "SSL security implementation",
  ];

  const industries = [
    "Private Medical Clinics",
    "NHS-affiliated Practices",
    "Dental Clinics",
    "Orthodontists",
    "Cosmetic Clinics",
    "Physiotherapists",
    "Chiropractors",
    "Mental Health Clinics",
    "Therapy Practices",
    "Opticians",
    "Care Homes",
    "Veterinary Practices",
    "Pharmacies",
    "Specialist Consultants",
  ];

  const benefits = [
    "Bespoke Website Design",
    "UK-Based Web Design Agency",
    "SEO-Friendly Development",
    "Mobile-First Design",
    "Fast Loading Websites",
    "Secure Development Practices",
    "Long-Term Website Support",
    "Transparent Communication",
    "Designed for Growth",
  ];

  return (
    <>
      <Navbar />
      <main>
        {/* Hero section */}
        <section className="relative bg-[#12372A] pt-24 pb-14 md:pt-28 md:pb-20 overflow-hidden">
          <div className="absolute top-0 left-0 right-0 h-[3px] bg-[#B08D57]" />

          <div className="relative max-w-[1320px] mx-auto px-6 md:px-10">
            <div className="grid md:grid-cols-[3fr_2fr] gap-12 items-center">
              {/* Left content */}
              <div>
                <div className="flex items-center gap-4 mb-8">
                  <span className="block w-8 h-[1px] bg-[#B08D57]" />
                  <span className="text-[#B08D57] text-[0.68rem] tracking-[0.22em] uppercase font-sans">
                    Healthcare
                  </span>
                </div>
                <h1
                  className="font-serif-display text-[#F6F1E8] leading-[1.08] mb-6"
                  style={{ fontSize: "clamp(2.2rem, 4vw, 3.2rem)" }}
                >
                  Healthcare Web Design for <em className="text-[#B08D57]">Modern Medical Practices</em>
                </h1>
                <p className="text-[#F6F1E8]/70 text-[0.9rem] leading-relaxed font-sans font-light mb-8 max-w-[52ch]">
                  Your website is often a patient's first impression of your practice. At <strong className="font-normal">Crea Web</strong>, we specialise in <strong className="font-normal">Healthcare Web Design</strong> that combines modern design, accessibility, security, and performance to help clinics, private practices, dental surgeries, and healthcare organisations build trust, attract new patients, and provide a seamless online experience.
                </p>
                <Link
                  href="/contact"
                  className="inline-flex items-center gap-3 text-[0.75rem] font-medium tracking-[0.14em] uppercase font-sans text-[#12372A] bg-[#F6F1E8] px-8 py-4 hover:bg-[#B08D57] hover:text-[#F6F1E8] transition-colors duration-300"
                >
                  Start Your Project
                  <svg width="14" height="10" viewBox="0 0 14 10" fill="none" aria-hidden="true">
                    <path d="M1 5h12M8 1l5 4-5 4" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </Link>
              </div>

              {/* Right image */}
              <div className="hidden md:block md:col-span-1 relative">
                <div className="absolute -top-4 -right-4 w-16 h-16 border-t border-r border-[#B08D57]/60 z-10" />
                <div className="absolute -bottom-4 -left-4 w-16 h-16 border-b border-l border-[#B08D57]/60 z-10" />
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src="/images/industries/health-hero.jpg"
                  alt="Healthcare Web Design UK"
                  className="w-full h-full object-cover rounded-sm"
                  loading="lazy"
                />
              </div>
            </div>
          </div>
        </section>

        {/* About section */}
        <section className="bg-[#F6F1E8] py-24 md:py-32">
          <Reveal>
            <div className="max-w-[1320px] mx-auto px-6 md:px-10">
              <div className="flex items-center gap-6 mb-16">
                <span className="text-[#B08D57] text-[0.68rem] tracking-[0.22em] uppercase font-sans">
                  About Healthcare Web Design
                </span>
                <hr className="flex-1 divider" />
              </div>

              <div className="grid lg:grid-cols-12 gap-12 lg:gap-20">
                {/* Left image */}
                <div className="lg:col-span-5 order-2 lg:order-1">
                  <div className="relative">
                    <div className="absolute -top-4 -left-4 w-16 h-16 border-t border-l border-[#B08D57]/60 z-10" />
                    <div className="absolute -bottom-4 -right-4 w-16 h-16 border-b border-r border-[#B08D57]/60 z-10" />
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src="/images/industries/health-about.jpg"
                      alt="Healthcare Web Design UK"
                      className="w-full h-full object-cover aspect-[3/4]"
                    />
                  </div>
                </div>

                {/* Right content */}
                <div className="lg:col-span-7 space-y-6 order-1 lg:order-2">
                  <h2 className="font-serif-display text-[#1C1C1C] leading-[1.1]" style={{ fontSize: "clamp(1.8rem, 3vw, 2.4rem)" }}>
                    Websites That Build <em className="text-[#B08D57]">Patient Confidence</em>
                  </h2>
                  <p className="text-[#1C1C1C] text-[1rem] leading-[1.82] font-sans font-light">
                    In healthcare, trust begins long before a patient walks through your doors. A professionally designed website reassures visitors, answers important questions, and makes it easy to book appointments or contact your team. At <strong className="font-normal">Crea Web</strong>, we create bespoke healthcare websites that reflect the professionalism of your practice while delivering an intuitive user experience across every device.
                  </p>
                  <p className="text-[#7C746A] text-[1rem] leading-[1.82] font-sans font-light">
                    Our websites are built with both patients and healthcare professionals in mind. Whether you operate a private medical practice, dental clinic, physiotherapy centre, mental health service, or specialist healthcare organisation, we design websites that are easy to navigate, mobile-friendly, and structured to help patients quickly find the information they need. Every project is developed with speed, accessibility, SEO, and long-term scalability at its core.
                  </p>
                  <p className="text-[#7C746A] text-[1rem] leading-[1.82] font-sans font-light">
                    As a trusted <strong className="font-normal">UK web design agency</strong>, we understand the importance of credibility in the healthcare sector. From secure enquiry forms and online appointment requests to service pages, practitioner profiles, and patient resources, we build websites that strengthen your reputation while supporting sustainable business growth.
                  </p>
                </div>
              </div>
            </div>
          </Reveal>
        </section>

        {/* Why Healthcare Providers Choose Crea Web */}
        <section className="bg-[#F6F1E8] py-24 md:py-32">
          <Reveal>
            <div className="max-w-[1320px] mx-auto px-6 md:px-10">
              <div className="flex items-center gap-6 mb-12 lg:mb-20">
                <span className="text-[#B08D57] text-[0.68rem] tracking-[0.22em] uppercase font-sans">
                  Why Choose Us
                </span>
                <hr className="flex-1 divider" />
              </div>

              <div className="grid lg:grid-cols-12 gap-8 lg:gap-12 mb-16">
                <div className="lg:col-span-7">
                  <h2 className="font-serif-display text-[#12372A] leading-[1.1]" style={{ fontSize: "clamp(1.8rem, 3vw, 2.4rem)" }}>
                    Why Healthcare Providers <em className="text-[#B08D57]">Choose Crea Web</em>
                  </h2>
                </div>
              </div>

              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-[#1C1C1C]/10 border border-[#1C1C1C]/10">
                {reasons.map((reason) => (
                  <div key={reason.title} className="bg-[#F6F1E8] p-8 md:p-10 flex flex-col gap-5">
                    <div className="w-10 h-10 flex items-center justify-center text-[#12372A]">
                      {reason.icon}
                    </div>
                    <div>
                      <p className="font-serif-display text-[#12372A] text-[1.2rem] mb-3">
                        {reason.title}
                      </p>
                      <p className="text-[#7C746A] text-[0.88rem] leading-[1.78] font-sans">
                        {reason.description}
                      </p>
                    </div>
                  </div>
                ))}
              </div>
            </div>
          </Reveal>
        </section>

        {/* Healthcare Website Features */}
        <section className="bg-[#F6F1E8] py-24 md:py-32">
          <Reveal>
            <div className="max-w-[1320px] mx-auto px-6 md:px-10">
              <div className="flex items-center gap-6 mb-16">
                <span className="text-[#B08D57] text-[0.68rem] tracking-[0.22em] uppercase font-sans">
                  Features
                </span>
                <hr className="flex-1 divider" />
              </div>

              <h2
                className="font-serif-display text-[#1C1C1C] leading-[1.1] mb-12"
                style={{ fontSize: "clamp(1.8rem, 3vw, 2.4rem)" }}
              >
                Healthcare Website <em className="text-[#B08D57]">Features</em>
              </h2>

              <p className="text-[#7C746A] text-[0.95rem] leading-[1.7] mb-10 max-w-[65ch] font-sans font-light">
                Every healthcare website can include:
              </p>

              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {features.map((feature, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <svg
                      className="w-5 h-5 text-[#12372A] flex-shrink-0 mt-0.5"
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke="currentColor"
                      strokeWidth="2"
                      aria-hidden="true"
                    >
                      <polyline points="20 6 9 17 4 12"></polyline>
                    </svg>
                    <p className="text-[#1C1C1C] text-[0.95rem] font-sans font-light">{feature}</p>
                  </div>
                ))}
              </div>
            </div>
          </Reveal>
        </section>

        {/* Industries We Work With */}
        <section className="bg-[#F6F1E8] py-24 md:py-32">
          <Reveal>
            <div className="max-w-[1320px] mx-auto px-6 md:px-10">
              <div className="flex items-center gap-6 mb-16">
                <span className="text-[#B08D57] text-[0.68rem] tracking-[0.22em] uppercase font-sans">
                  Specialisms
                </span>
                <hr className="flex-1 divider" />
              </div>

              <h2
                className="font-serif-display text-[#1C1C1C] leading-[1.1] mb-12"
                style={{ fontSize: "clamp(1.8rem, 3vw, 2.4rem)" }}
              >
                Industries We <em className="text-[#B08D57]">Work With</em>
              </h2>

              <p className="text-[#7C746A] text-[0.95rem] leading-[1.7] mb-10 max-w-[65ch] font-sans font-light">
                We design websites for:
              </p>

              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {industries.map((industry, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <span className="block w-2 h-2 bg-[#12372A] rounded-full"></span>
                    <p className="text-[#1C1C1C] text-[0.95rem] font-sans font-light">{industry}</p>
                  </div>
                ))}
              </div>
            </div>
          </Reveal>
        </section>

        {/* Why Modern Website Matters */}
        <section className="bg-[#F6F1E8] py-24 md:py-32">
          <Reveal>
            <div className="max-w-[1320px] mx-auto px-6 md:px-10">
              <div className="flex items-center gap-6 mb-16">
                <span className="text-[#B08D57] text-[0.68rem] tracking-[0.22em] uppercase font-sans">
                  Market Need
                </span>
                <hr className="flex-1 divider" />
              </div>

              <div className="grid lg:grid-cols-12 gap-12 lg:gap-20">
                {/* Left content */}
                <div className="lg:col-span-7 space-y-6">
                  <h2
                    className="font-serif-display text-[#1C1C1C] leading-[1.1] mb-8"
                    style={{ fontSize: "clamp(1.8rem, 3vw, 2.4rem)" }}
                  >
                    Why Your Healthcare Practice Needs a <em className="text-[#B08D57]">Modern Website</em>
                  </h2>

                  <p className="text-[#1C1C1C] text-[1rem] leading-[1.82] font-sans font-light">
                    Today's patients expect more than basic information—they expect convenience, speed, and confidence. A professionally designed healthcare website makes it easier for prospective patients to discover your services, understand your expertise, and take the next step by booking an appointment or making an enquiry. An outdated website can create uncertainty, while a modern, well-designed website reinforces professionalism and trust from the very first visit.
                  </p>
                  <p className="text-[#1C1C1C] text-[1rem] leading-[1.82] font-sans font-light">
                    At Crea Web, we combine strategic design with technical excellence to create healthcare websites that not only look exceptional but also perform effectively in search engines. Every website is developed with SEO best practices, responsive layouts, and intuitive navigation, helping your practice reach more patients while providing an outstanding digital experience.
                  </p>
                </div>

                {/* Right image */}
                <div className="hidden lg:block lg:col-span-5 relative">
                  <div className="absolute -top-4 -right-4 w-16 h-16 border-t border-r border-[#B08D57]/60 z-10" />
                  <div className="absolute -bottom-4 -left-4 w-16 h-16 border-b border-l border-[#B08D57]/60 z-10" />
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src="/images/industries/health-why.jpg"
                    alt="Healthcare Web Design UK"
                    className="w-full h-full object-cover aspect-[3/4]"
                  />
                </div>
              </div>
            </div>
          </Reveal>
        </section>

        {/* Why Crea Web */}
        <section className="bg-[#F6F1E8] py-24 md:py-32">
          <Reveal>
            <div className="max-w-[1320px] mx-auto px-6 md:px-10">
              <div className="flex items-center gap-6 mb-16">
                <span className="text-[#B08D57] text-[0.68rem] tracking-[0.22em] uppercase font-sans">
                  Our Promise
                </span>
                <hr className="flex-1 divider" />
              </div>

              <h2
                className="font-serif-display text-[#1C1C1C] leading-[1.1] mb-12"
                style={{ fontSize: "clamp(1.8rem, 3vw, 2.4rem)" }}
              >
                Why <em className="text-[#B08D57]">Crea Web?</em>
              </h2>

              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {benefits.map((benefit, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <svg
                      className="w-5 h-5 text-[#B08D57] flex-shrink-0"
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke="currentColor"
                      strokeWidth="2"
                      aria-hidden="true"
                    >
                      <polyline points="20 6 9 17 4 12"></polyline>
                    </svg>
                    <p className="text-[#1C1C1C] text-[0.95rem] font-sans font-light">{benefit}</p>
                  </div>
                ))}
              </div>
            </div>
          </Reveal>
        </section>

        {/* Pricing Section */}
        <section className="bg-white py-24 md:py-32">
          <Reveal>
            <div className="max-w-[1320px] mx-auto px-6 md:px-10">
              <div className="flex items-center gap-6 mb-16">
                <span className="text-[#B08D57] text-[0.68rem] tracking-[0.22em] uppercase font-sans">
                  Investment
                </span>
                <hr className="flex-1 divider" />
              </div>

              <div className="mb-20">
                <h2 className="font-serif-display text-[#1C1C1C] leading-[1.1] mb-6" style={{ fontSize: "clamp(1.8rem, 3vw, 2.4rem)" }}>
                  Healthcare Website <em className="text-[#B08D57]">Pricing</em>
                </h2>
                <p className="text-[#7C746A] text-[1rem] leading-[1.82] font-sans font-light max-w-[70ch]">
                  Transparent, bespoke pricing for healthcare practices of all sizes. Every package is tailored to your needs with no hidden fees.
                </p>
              </div>

              {/* Pricing Tiers */}
              <div className="grid md:grid-cols-3 gap-6 mb-20">
                {/* Foundation Tier */}
                <div className="bg-[#F6F1E8] border border-[#1C1C1C]/10 p-8 flex flex-col gap-8">
                  <div>
                    <p className="text-[#B08D57] text-[0.75rem] tracking-[0.14em] uppercase font-sans font-medium mb-3">
                      Foundation
                    </p>
                    <div className="flex items-baseline gap-2 mb-2">
                      <span className="font-serif-display text-[#12372A] text-[2.2rem]">£2,500</span>
                    </div>
                    <p className="text-[#7C746A] text-[0.85rem] font-sans">
                      One-time project fee
                    </p>
                  </div>
                  <div className="space-y-3 flex-1">
                    {["Up to 8 pages", "Responsive design", "Basic SEO setup", "Contact forms", "Mobile optimized"].map((feature, i) => (
                      <div key={i} className="flex items-start gap-3">
                        <svg className="w-4 h-4 text-[#B08D57] flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" aria-hidden="true"><polyline points="20 6 9 17 4 12"></polyline></svg>
                        <span className="text-[#1C1C1C] text-[0.9rem] font-sans">{feature}</span>
                      </div>
                    ))}
                  </div>
                  <Link href="/contact" className="justify-center inline-flex items-center gap-3 text-[0.75rem] font-medium tracking-[0.14em] uppercase font-sans text-[#F6F1E8] bg-[#12372A] px-6 py-3 hover:bg-[#B08D57] hover:text-[#12372A] transition-colors duration-300 w-full">
                    Get Started
                  </Link>
                </div>

                {/* Commerce Tier */}
                <div className="bg-[#12372A] border-2 border-[#B08D57] p-8 flex flex-col gap-8">
                  <div>
                    <p className="text-[#B08D57] text-[0.75rem] tracking-[0.14em] uppercase font-sans font-medium mb-3">
                      Professional
                    </p>
                    <div className="flex items-baseline gap-2 mb-2">
                      <span className="font-serif-display text-[#F6F1E8] text-[2.2rem]">£7,500</span>
                    </div>
                    <p className="text-[#F6F1E8]/70 text-[0.85rem] font-sans">
                      One-time project fee
                    </p>
                  </div>
                  <div className="space-y-3 flex-1">
                    {["Up to 15 pages", "Appointment booking", "Practitioner profiles", "Advanced SEO", "Blog setup", "Analytics tracking"].map((feature, i) => (
                      <div key={i} className="flex items-start gap-3">
                        <svg className="w-4 h-4 text-[#B08D57] flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" aria-hidden="true"><polyline points="20 6 9 17 4 12"></polyline></svg>
                        <span className="text-[#F6F1E8] text-[0.9rem] font-sans">{feature}</span>
                      </div>
                    ))}
                  </div>
                  <Link href="/contact" className="justify-center inline-flex items-center gap-3 text-[0.75rem] font-medium tracking-[0.14em] uppercase font-sans text-[#12372A] bg-[#B08D57] px-6 py-3 hover:bg-[#F6F1E8] transition-colors duration-300 w-full">
                    Get Started
                  </Link>
                </div>

                {/* Enterprise Tier */}
                <div className="bg-[#F6F1E8] border border-[#1C1C1C]/10 p-8 flex flex-col gap-8">
                  <div>
                    <p className="text-[#B08D57] text-[0.75rem] tracking-[0.14em] uppercase font-sans font-medium mb-3">
                      Enterprise
                    </p>
                    <div className="flex items-baseline gap-2 mb-2">
                      <span className="font-serif-display text-[#12372A] text-[2.2rem]">£15,000+</span>
                    </div>
                    <p className="text-[#7C746A] text-[0.85rem] font-sans">
                      One-time project fee
                    </p>
                  </div>
                  <div className="space-y-3 flex-1">
                    {["Unlimited pages", "Multi-location support", "Custom integrations", "Advanced security", "Staff portal", "Patient resources", "Custom features"].map((feature, i) => (
                      <div key={i} className="flex items-start gap-3">
                        <svg className="w-4 h-4 text-[#12372A] flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" aria-hidden="true"><polyline points="20 6 9 17 4 12"></polyline></svg>
                        <span className="text-[#1C1C1C] text-[0.9rem] font-sans">{feature}</span>
                      </div>
                    ))}
                  </div>
                  <Link href="/contact" className="justify-center inline-flex items-center gap-3 text-[0.75rem] font-medium tracking-[0.14em] uppercase font-sans text-[#F6F1E8] bg-[#12372A] px-6 py-3 hover:bg-[#B08D57] hover:text-[#12372A] transition-colors duration-300 w-full">
                    Get Started
                  </Link>
                </div>
              </div>


            </div>
          </Reveal>
        </section>

        {/* FAQ Section */}
        <section className="bg-[#F6F1E8] py-24 md:py-32">
          <Reveal>
            <div className="max-w-[1320px] mx-auto px-6 md:px-10">
              <div className="flex items-center gap-6 mb-16">
                <span className="text-[#B08D57] text-[0.68rem] tracking-[0.22em] uppercase font-sans">
                  Questions
                </span>
                <hr className="flex-1 divider" />
              </div>

              <h2
                className="font-serif-display text-[#1C1C1C] leading-[1.1] mb-12"
                style={{ fontSize: "clamp(1.8rem, 3vw, 2.4rem)" }}
              >
                Frequently Asked <em className="text-[#B08D57]">Questions</em>
              </h2>

              <div className="space-y-8 max-w-[80ch]">
                {faqs.map((faq, i) => (
                  <div key={i} className="border-b border-[#1C1C1C]/10 pb-8">
                    <h3 className="text-[#12372A] font-serif-display text-[1.2rem] leading-tight mb-3">
                      {faq.q}
                    </h3>
                    <p className="text-[#7C746A] text-[0.95rem] leading-[1.7] font-sans font-light">
                      {faq.a}
                    </p>
                  </div>
                ))}
              </div>
            </div>
          </Reveal>
        </section>

        {/* CTA Section */}
        <section className="bg-[#12372A] py-24 md:py-32">
          <Reveal>
            <div className="max-w-[900px] mx-auto px-6 md:px-10">
              <div className="flex flex-col items-center text-center">
                <div className="mb-10">
                  <h2
                    className="font-serif-display text-[#F6F1E8] leading-[1.08] mb-6"
                    style={{ fontSize: "clamp(2rem, 3.5vw, 2.8rem)" }}
                  >
                    Build a Website Your <em className="text-[#B08D57]">Patients Can Trust</em>
                  </h2>
                  <p className="text-[#F6F1E8]/80 text-[1rem] leading-[1.82] font-sans font-light mb-6">
                    Whether you're launching a new healthcare practice or modernising an existing website, <strong className="font-normal">Crea Web</strong> designs secure, SEO-friendly healthcare websites that help you attract more patients and grow with confidence.
                  </p>
                  <p className="text-[#F6F1E8] text-[1rem] leading-[1.82] font-sans font-light">
                    <strong className="font-normal">Ready to transform your online presence? Get in touch today for a free consultation.</strong>
                  </p>
                </div>
                <Link
                  href="/contact"
                  className="inline-flex items-center gap-3 text-[0.75rem] font-medium tracking-[0.14em] uppercase font-sans text-[#12372A] bg-[#B08D57] px-8 py-4 hover:bg-[#F6F1E8] transition-colors duration-300"
                >
                  Request a Consultation
                  <svg width="14" height="10" viewBox="0 0 14 10" fill="none" aria-hidden="true">
                    <path d="M1 5h12M8 1l5 4-5 4" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </Link>
              </div>
            </div>
          </Reveal>
        </section>
      </main>
      <Footer />
    </>
  );
}

...
export default function InsightCover({ post, eager = false }) {
  ...
  if (placeholder) return <div className="ij-cover-art" data-category={post.category} aria-hidden="true"><span>Crea Web / Journal</span><b>{post.category || "Ideas & insights"}</b><span className="ij-cover-symbol">{post.category === "Typography" ? "Aa" : post.category === "Strategy" ? "↗" : "✳"}</span><span>{post.issue || "Ideas worth sharing"}</span></div>;

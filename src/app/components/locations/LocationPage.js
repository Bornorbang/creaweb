    ...
    <main id="location-content">
      <section className="cl-hero" aria-labelledby="location-title">
        <div className="cl-shell">
          ...
          <div className="cl-hero-grid"><div className="cl-hero-copy"><span className="cw-label">Crea Web Agency / {city}</span><h1 id="location-title">Web design.<br /><span>{city}.</span></h1><p className="cl-hero-statement">{location.heroTitle}</p><p>{location.intro}</p><div className="cl-actions"><Action>{location.cta}</Action><Link className="cl-text-link" href="#work">Explore our work <span aria-hidden="true">↘</span></Link></div></div>
          <figure className="cl-city-photo"><div className="cl-city-image"><Image src={locationImages[slug]} alt={`${city} cityscape — web design services for ${city} businesses`} fill sizes="(max-width: 850px) 100vw, 46vw" priority style={{objectFit:"cover"}} /></div><figcaption><span>{city}, United Kingdom</span><span aria-hidden="true">↗</span></figcaption><span className="cl-city-stamp" aria-hidden="true">LOCAL<br />AMBITION.<br /><b>BIG IDEAS.</b></span></figure></div>
        </div>
      </section>

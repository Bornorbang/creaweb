...
function Button({ children = "Discuss your project", href = "/contact", secondary = false }) {
  return <Link href={href} className={`cw-button${secondary ? " ci-secondary" : ""}`}>{children}<span aria-hidden="true">↗</span></Link>;
}
    ...
    <main id="industry-content">
      <section className="ci-hero ci-shell" aria-labelledby="industry-title">
        ...
        <div className="ci-hero-grid">
          <div className="ci-hero-copy"><span className="cw-label">Different industries. Individual ideas.</span><h1 id="industry-title">{d.label}<br /><span>web design.</span></h1><p className="ci-statement">{d.heroTitle}</p><p>{d.intro}</p><div className="ci-actions"><Button>{d.cta || "Discuss your project"}</Button><a className="ci-text-link" href="#work">See our work <span aria-hidden="true">↘</span></a></div></div>
          <div className="ci-hero-art"><div className="ci-image-toolbar"><span>Crea Web Agency</span><span aria-hidden="true">● ● ●</span></div><Photo image={d.images[0]} keyword={d.keyword} context="industry inspiration" priority /><div className="ci-stamp" aria-hidden="true">YOUR SECTOR.<br />OUR CRAFT.<span>✳</span></div></div>
        </div>
      </section>
      ...
      <section id="services" className="ci-reasons"><div className="ci-shell ci-section"><div className="ci-heading"><div><span className="cw-label">02 / Built with purpose</span><h2>{d.reasonsTitle}</h2></div><span className="ci-large-star" aria-hidden="true">✳</span></div><div className="ci-reason-grid">{d.reasons.map((r, i) => <article key={r.title}><span className="ci-number" aria-hidden="true">{String(i + 1).padStart(2, "0")}</span><div><h3>{r.title}</h3><p>{r.body}</p></div></article>)}</div></div></section>

      <section className="ci-shell ci-section ci-features"><div className="ci-feature-title"><span className="cw-label">03 / The details that matter</span><h2>{d.featuresTitle}</h2>{d.featuresIntro && <p>{d.featuresIntro}</p>}<span className="ci-feature-arrow" aria-hidden="true">↗</span></div><ul>{d.features.map((f, i) => <li key={f}><span aria-hidden="true">{String(i + 1).padStart(2, "0")}</span>{f}</li>)}</ul></section>

      <section className="ci-sectors"><div className="ci-shell ci-section"><div className="ci-heading"><div><span className="cw-label">04 / Your kind of business</span><h2>{d.sectorsTitle}</h2></div>{d.sectorsIntro && <p>{d.sectorsIntro}</p>}</div><ul>{d.sectors.map(s => <li key={s}>{s}<span aria-hidden="true">↗</span></li>)}</ul></div></section>
      ...
      <section className="ci-shell ci-benefits"><span className="cw-label">The Crea Web approach</span><h2>{d.benefitsTitle}</h2><ul>{d.benefits.map(b => <li key={b}><span aria-hidden="true">✓</span>{b}</li>)}</ul></section>

      <section id="work" className="ci-shell ci-section ci-work"><div className="ci-heading"><div><span className="cw-label">06 / From the studio</span><h2>Good ideas.<br />Out in the world.</h2></div><div><p>A selection of websites from across our portfolio.</p><Link className="ci-text-link" href="/projects">Explore all projects ↗</Link></div></div><div className="ci-projects">{projects.map(p => <Link href="/projects" key={p.index} className="ci-project"><div><Image src={p.image} alt={`${p.client} website — ${d.keyword} UK portfolio`} fill sizes="(max-width: 600px) 90vw, 30vw" style={{ objectFit: "cover", objectPosition: "top" }} /><span aria-hidden="true">↗</span></div><h3>{p.client}</h3><p>{p.category}</p></Link>)}</div></section>
      ...
      <section id="questions" className="ci-shell ci-section ci-faq"><div><span className="cw-label">08 / Before we get started</span><h2>Your questions.<br />Straight answers.</h2><p>Frequently asked questions about {d.keyword.toLowerCase()}.</p><Link className="ci-text-link" href="/contact">Ask us something else ↗</Link></div><div>{d.faqs.map(f => <details key={f.q}><summary>{f.q}<span aria-hidden="true">+</span></summary><p>{f.a}</p></details>)}</div></section>

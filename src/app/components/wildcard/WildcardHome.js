...
function BrowserFrame({ project = projects[0], className = "", priority = false }) {
  return <figure className={`cw-browser ${className}`}><div className="cw-browser-bar"><span aria-hidden="true">● ● ●</span><span>{project.client}</span><span aria-hidden="true"><SymbolIcon symbol="↗" /></span></div><ModernImage src={project.image} alt={`${project.client} homepage showing bespoke website design by Crea Web`} top priority={priority} /></figure>;
}

function ModernHero() {
 const priority = true;
  return <section className="cw-hero cw-wildcard-hero"><div className="cw-wild-intro"><span className="cw-label">CREA WEB / UK WEB DESIGN STUDIO</span><h1>Professional <br />Web Design <span className="cw-hero-location">Agency UK<span className="cw-wild-period">.</span></span></h1><div className="cw-wild-bottom"><span className="cw-wild-star" aria-hidden="true"><SymbolIcon symbol="✳" /></span><p>{copy.intro}</p><Button>LET’S MAKE IT HAPPEN</Button></div></div><div className="cw-wild-projects"><div className="cw-wild-caption"><span>GOOD DESIGN.<br />REAL BUSINESS RESULTS.</span><Link href="#work">OUR WORK <SymbolIcon symbol="↘" /></Link></div><BrowserFrame project={projects[2]} priority={priority} /><div className="cw-wild-seal">DESIGNED<br />TO STAND<br /><strong>OUT <SymbolIcon symbol="↗" /></strong></div></div><div className="cw-wild-ticker" aria-hidden="true"><span>DESIGN <SymbolIcon symbol="✳" /> DEVELOPMENT <SymbolIcon symbol="✳" /> E-COMMERCE <SymbolIcon symbol="✳" /> SEO <SymbolIcon symbol="✳" /> DESIGN <SymbolIcon symbol="✳" /> DEVELOPMENT <SymbolIcon symbol="✳" /></span></div></section>;
}

function Services() {
  const icons = ["✳", "⌘", "✦", "↗", "◎", "∞"];
  return <section id="services" className="cw-section cw-services"><div className="cw-section-heading"><div><span className="cw-label">What we do</span><h2>Web design services<br />in the UK.</h2></div><p>Website design services for UK businesses, from bespoke web design to e-commerce and ongoing support. One team to plan, design and build your next website.</p></div><div className="cw-service-grid">{services.map((s,i)=><Link className="cw-service-card" key={s.title} href={s.href}><div><span className="cw-service-icon" aria-hidden="true">{["✳", "↗"].includes(icons[i]) ? <SymbolIcon symbol={icons[i]} /> : icons[i]}</span><span aria-hidden="true"><SymbolIcon symbol="↗" /></span></div><h3>{s.title}</h3><p>{s.body}</p></Link>)}</div></section>;
}
